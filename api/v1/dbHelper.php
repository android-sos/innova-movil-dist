...
class dbHelper {
    ...
    function select($table, $columns, $where, $operador, $order){
        try{
            $a = array();
            $w = "";
            $sql = "";
            $w = $this->makeWhere($where, $operador);

            $sql = "select ".$columns." from ".$table." where ". $w." ".$order;
    ...
    function insert($table, $columnsArray, $requiredColumnsArray, $pg_secuencial) {
        ...
        try{
            ...
            //SOLO PARA POSTGRESS
            // Colocar en la columna serial el secuencial. Big Serial. 
            if(strlen(trim($pg_secuencial))>0){
                $lastInsertId = $this->db->lastInsertId($pg_secuencial);
            }  
    ...
    function makeWhere($where, $operador){
        $a = array();
        $w = " 1=1 ";
        
        if(strlen(trim($operador)) == 0){
            $operador = 'AND';      
        }    

        if(is_array($where)){
            foreach ($where as $key => $value) {
                $w .= " ".$operador." " .$key. " like ".$value;
                $a[":".$key] = $value;
            }
        return $w;
        }
